...
public class myBoidFlock {
	...
	public void initFlock(){
		boidFlock = new myBoid[numBoids];
		for(int c = 0; c < boidFlock.length; ++c){
			boidFlock[c] = new myBoid(p,this,/*nStenc,*/ randBoidStLoc(1), type, fv);
		//	boidFlock[c].setInitState();
		}
	...
	public void modBoidPop(int m){
		if(m>0){if(boidFlock.length >= p.maxBoats) {return;}for(int i=0;i<m;++i){ addBoid();}} 
	...
	public myBoid addBoid(myPoint stLoc){
		...
		for(int i=0;i<boidFlock.length;++i){tmpList[i]=boidFlock[i];}
	...
	public void removeBoid(){removeBoid(boidFlock.length-1);}
	public void removeBoid(int idx){
		if(idx<0){return;}	
		myBoid tmpList[] = new myBoid[numBoids-1];
		for(int i=0;i<idx;++i){tmpList[i]=boidFlock[i];}
		for(int i=idx+1;i<boidFlock.length;++i){tmpList[i-1]=boidFlock[i];}	
		numBoids--;	
		boidFlock = tmpList;
	}//removeBoid
	...
	public void removeBoids(ArrayList<Integer> dead){
		...
		for(int i=0; i<boidFlock.length; ++i){if(!dead.contains(i)){tmpList.add(boidFlock[i]);}	}
		boidFlock = tmpList.toArray(new myBoid[0]);	
	}

	//move creatures to random start positions
	public void scatterBoids() {for(int c = 0; c < boidFlock.length; ++c){boidFlock[c].coords[0] =  randBoidStLoc(1);}}//	randInit
//	public void drawBoids(){
//		p.pushMatrix();p.pushStyle();
//		p.translate(-p.gridDimW/2.0f,-p.gridDimDp/2.0f,-p.gridDimH/2.0f);
//  		p.strokeWeight(.5f);
//  		for(int c = 0; c < boidFlock.length; ++c){boidFlock[c].drawMe(); } 
...
//		int i = 0;
//		for(int c = 0; c < boidFlock.length; ++c){res = boidFlock[c].getSimData();for(int j = 0; j<res.length; ++j){retVal[i++] = res[j];}}		
//		return retVal;
//	}//getFwdSimVals
//	
//	public void updateBoidsFromGLSL(int[] data){
//		int offset = 6;//# of variables to set
//		for(int c = 0; c < boidFlock.length; ++c){boidFlock[c].setGLSLData(data, c*offset);}
//	}
	
	//clear out last time step boid values
	public void clearOutBoids(){for(int c = 0; c < boidFlock.length; ++c){boidFlock[c].clearAllBoidMaps();}	}					//clear out all neighbors, preds and prey	
	public void clearBoidForces(){for(int c = 0; c < boidFlock.length; ++c){boidFlock[c].forces[0].set(0,0,0);}}	
	...
	public void moveBoidsMultTH(double _delT){
		...
		for(int c = 0; c < boidFlock.length; c+=mtFrameSize){
			...
			tmpList = new myBoid[finalLen];
			System.arraycopy(boidFlock, c, tmpList, 0, finalLen);			
			callFwdBoidCalcs.add(new myFwdStencil(p, this, preyFlock, predFlock, fv, tmpList));
		}							//find next turn's motion for every creature by finding total force to act on creature
	...
	public void updateBoidMovement(double _delT){
		...
		for(int c = 0; c < boidFlock.length; c+=mtFrameSize){
			...
			tmpList = new myBoid[finalLen];
			System.arraycopy(boidFlock, c, tmpList, 0, finalLen);			
			//p.th_exec.execute(new myUpdateStencil(p, this, boidFlock[c]));
			callUbdBoidCalcs.add(new myUpdateStencil(p, this, tmpList));
		}							//apply update
        ...
        for(int c = 0; c < boidFlock.length; ++c){
          if((boidFlock[c]!= null) && (boidFlock[c].bd_flags[myBoid.hasStarved])){    removeBoid(c);  }
          else {  
        	  //bl[0] = new myPoint(); bVelFrc[0]=new myVector(); bVelFrc[1]=new myVector(); 
        	  if(boidFlock[c].hadAChild(bl,bVelFrc)){myBoid tmpBby = this.addBoid(bl[0]); tmpBby.initNewborn(bVelFrc);}}
        } 
	}//updateBoids	
	...
	public String toString(){
		String res = "Flock Size " + boidFlock.length + "\n";
