...
class myPointf {
	...
	public void set(myPointf p){ this.x = p.x; this.y = p.y; this.z = p.z; }                                                                   //set 1 args
	...
	public float _L1Dist(myPointf q){return (float) Math.abs((this.x - q.x) + (this.y - q.y) + (this.z - q.z)); }
	public static float _L1Dist(myPointf q, myPointf r){ return (float) Math.abs((r.x - q.x) + (r.y - q.y) + (r.z - q.z));}
...
//	public static final myVector FORWARD = new myVector(0,0,1);
	...
	public static double _det3(myVector U, myVector V) {double tmp = U._dot(V);return (Math.sqrt(U._dot(U)*V._dot(V) - (tmp*tmp))); };                                // U|V det product
...
//	public static final myVectorf FORWARD = new myVectorf(0,0,1);
	...
	myVectorf(myPointf a){this(a.x,a.y,a.z);}			//vector from a->b
	...
	public myVectorf _normalize(){this._mag();if(magn==0){return this;}this.x = this.x/magn; this.y = this.y/magn; this.z = this.z/magn; return this;}
	...
	public float _L1Dist(myVectorf q){return (float) Math.abs((this.x - q.x) + (this.y - q.y) + (this.z - q.z)); }
	public static float _L1Dist(myVectorf q, myVectorf r){ return (float) Math.abs((r.x - q.x) + (r.y - q.y) + (r.z - q.z));}
	...
	public static float _det3(myVectorf U, myVectorf V) {float tmp = U._dot(V);return ((float) Math.sqrt(U._dot(U)*V._dot(V) - (tmp*tmp))); };                                // U|V det product
	public static float _mixProd(myVectorf U, myVectorf V, myVectorf W) {return U._dot(myVectorf._cross(V,W)); };                                                 // (UxV)*W  mixed product, determinant - measures 6x the volume of the parallelapiped formed by myVectorftors
